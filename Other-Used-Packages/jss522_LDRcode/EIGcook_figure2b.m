...
sig=1;
sig0=1;
ncols=10; nreps=50; u=1;
ang=zeros(5,length(siggy),nreps);
...
for k=1:length(siggy)
    ...
    for j=1:nreps
      ...
      [f,W,st] = cise_cv(Y,X,1,lambdas,nfolds,'EPFC');
      ang(6,k,j)=subspace(W,alp)*180/pi;

    end
end
  
angmean=zeros(5,length(siggy));
...
plot(siggy,squeeze(angmean(1,:)),'-',siggy,squeeze(angmean(2,:)),'-',siggy,squeeze(angmean(3,:)),'-',siggy,squeeze(angmean(4,:)),'-',siggy,squeeze(angmean(5,:)),'-')
% labels
