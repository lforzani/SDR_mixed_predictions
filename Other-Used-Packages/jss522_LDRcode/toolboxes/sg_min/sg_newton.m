...
% role	high level algorithm, Newton's Methods
	...
	gsdr = ip(Y,sdr,g);
	sa = -f/gsdr;
	sa = fminbnd_vDT(fun,-2*sa,2*sa,[],Y,sdr);
	sdrHsdr = ip(Y,sdr,dgrad(Y,sdr));
	sb = -gsdr/abs(sdrHsdr);
	sb = fminbnd_vDT(fun,-2*sb,2*sb,[],Y,sdr);
	Ysa = move(Y,sdr,sa);
	Ysb = move(Y,sdr,sb);
...
% algorithm.
	...
	nb = -gndr/abs(ndrHndr);
	nb = fminbnd_vDT(fun,-2*nb,2*nb,[],Y,ndr);
	Yna = move(Y,ndr,na);
	Ynb = move(Y,ndr,nb);
    ...
    Yn = Y;
    if (N==maxiter)
        warning('SGMIN:maxiter','reached maximum number of iterations without convergence for specified tolerances');
