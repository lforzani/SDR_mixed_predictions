...
%     nfolds=10;
%     lambdas = .001:.005:1;
    [f,beta,st]=cise(y,X,u,.005,'AIDA');
    beta = aida(y,X(:,find(st)));
   ...
   [WX, W]=ldr(y,X(:,find(st)),'lad','cont',u,'nslices',h);
    angulos(j,a,7)=subspace(W,alp)*180/pi;
  end
end
...
xlabel('a');
ylabel('ANGLE');
legend('LAD','SIR','SAVE','DR','AIDA','penAIDA','initLAD','Location','Best');
