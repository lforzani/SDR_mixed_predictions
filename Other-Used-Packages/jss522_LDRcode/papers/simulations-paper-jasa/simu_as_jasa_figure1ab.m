...
nrows=[15 30 40 50 80 120];               % sample sizes
...
%%%%%
    ...
    for p=1:4
      ...
      [WX,W]=ldr(Y,squeeze(X(p,:,:)),'HLDA','disc',u);
      compang_hlda(p,j,k)=subspace(W,alp(1,:)')*180/pi;

      XX = squeeze(X(p,:,:));
      [Wx,W]=eigHLDA(Y,XX,u,'disc');
      compang_eigHLDA(p,j,k)=subspace(W,alp(1,:)')*180/pi;

      [WX,W]=ldr(Y,squeeze(X(p,:,:)),'HLDA','disc',u,'initval',W);
      compang_hlda2(p,j,k)=subspace(W,alp(1,:)')*180/pi;
...
for p=1:4
    for k = 1:length(nrows),
        ...
        quantang_hlda(p,k,:) = quantile(compang_hlda(p,:,k),quant)';
        quantang_eigHLDA(p,k,:) = quantile(compang_eigHLDA(p,:,k),quant)';
        quantang_hlda2(p,k,:) = quantile(compang_hlda2(p,:,k),quant)';
    end
end
...
% conditional on Y.
figure(1);
plot(nrows,squeeze(quantang_lad(1,:,:))','-',nrows,squeeze(quantang_hlda(1,:,:))','--',nrows,squeeze(quantang_eigHLDA(1,:,:))','--',nrows,squeeze(quantang_hlda2(1,:,:))','--');
title('Normal error');
xlabel('n_{y}'); ylabel('Median angle');
legend('LAD','HLDA','eigHLDA','HLDA2');
ylim([0 90]);
