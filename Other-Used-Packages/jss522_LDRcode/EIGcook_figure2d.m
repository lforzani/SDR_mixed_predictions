%%% This is Figure 2d from R. D. Cook paper: Fisher Lecture: Dimension
...
% the PFC estimate is obtained from the whole candidate set. The average angle
...
ang=zeros(4,length(sigg0),nreps);
...
plot(sigg0,squeeze(angmean(1,:)),'-',sigg0,squeeze(angmean(2,:)),'-',sigg0 ,squeeze(angmean(3,:)),'-',sigg0 ,squeeze(angmean(4,:)),'-',sigg0 ,squeeze(angmean(5,:)),'-')
% labels
legend('SIR','OLS','EPFC','eigEPFC','eigINIT');
