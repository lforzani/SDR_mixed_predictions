...
% function [WX,W,auxmtx] = SIR(Y,X,morph,u,varargin)
% 
...
%     used to compute SAVE and DR reductions.
%   - inputs:
%     Y: response vector.
%     X: predictors matrix.
...
%     SETAUX, from which the SIR reduction can be easily extracted. This is
%     useful when SIR is used along with SAVE and DR, as they share some
...
[SIRdir,SIR] = getSIR(dim,parameters.auxhandle);

%-----Write results------------------------
W = SIR;
WX = (X-repmat(mean(X),rows(X),1))*W;
