function [Wn,fn,fp] = pfc(Yaux,X,u,morph,parameters)
% [Wn,fn,fp] = pfc(Y,X,u,morph,parameters)
%
...
%     [Wn,vals] = firsteigs(inv(SIGMAres)*SIGMA,u);
    [Wn,vals] = firsteigs(invsqrtm(SIGMAres)*SIGMA*invsqrtm(SIGMAres),u);
    Wn = orth(invsqrtm(SIGMAres)*Wn);
    fn = Fhandle(vals);
end
