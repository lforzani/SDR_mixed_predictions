clear all;
%Simulation for mixed predictors with d=2 
n = 2000;
    ...
    for mm = 1:length(k)
        ...
        [red_est_suboptimal1,red_est_suboptimal2,betahat] = rr4mix_suboptimal(X,H,Y,d1,d2,'disc','auto');
              ...
              red_est_optimalCONT = red_est_optimal(1:p,:);
               ...
               red_est_optimalBIN1 = red_est_optimalBIN(:,1);
               red_est_optimalBIN2 = red_est_optimalBIN(:,2);
               
               
               for i=1:q
                   
                   if red_est_optimalBIN1(indexZ(i,:))==0 & red_est_optimalBIN2(indexZ(i,:))==0;
                       CountZerosBIN(i) =1;
                   else
                       CountZerosBIN(i)=0;
                   end
         ...
         %To count coincidence in total zeros (no seleceted variables)
...
csvwrite('Mixed_estimator_dim2_optimal.csv',Roptimalauto_2);
csvwrite('Mixed_proj_dim2_optimal.csv',PDoptimalauto_2);
         
csvwrite('Mixed_estimator_dim2_suboptimal_cont.csv',RDsubop1auto_2);
csvwrite('Mixed_estimator_dim2_suboptimal_disc.csv',RDsubop2auto_2);
csvwrite('Mixed_proj_dim2_suboptimal_cont.csv',PDsuboptimalauto_cont_2);
csvwrite('Mixed_proj_dim2_suboptimal_disc.csv',PDsuboptimalauto_disc_2);

csvwrite('Mixed_estimator_dim2_PCA.csv',RD_PCA_2);
csvwrite('Mixed_proj_dim2_optimal.csv',PD_PCA_2);
 
csvwrite('Mixed_estimator_dim2_PFC.csv',RD_PFC_2);
csvwrite('Mixed_proj_dim2_PFC.csv',PD_PFC_2);
